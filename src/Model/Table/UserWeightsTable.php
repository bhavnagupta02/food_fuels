...
class UserWeightsTable extends Table
{
    ...
    public function findAndUpdateMyLoss($UserID = null){
        if(!empty($UserID)){
            ...
            $totalWeightLoss = ($MaxWeightTotal != 0) ? $MaxWeightTotal-$currentWeight : 0.00;
            $totalWeightLossPercentage = ($totalWeightLoss != 0.00) ? round(($totalWeightLoss*100)/$MaxWeightTotal,0) : 0.00;
            
            $weekWeightLoss = ($MaxWeightWeek != 0) ? $MaxWeightWeek-$currentWeight : 0.00;
            $WeekWeightLossPercentage = ($weekWeightLoss != 0.00) ? round(($weekWeightLoss*100)/$MaxWeightWeek,0) : 0.00;
            
	    $monthWeightLoss = ($MaxWeightMonth != 0) ? $MaxWeightMonth-$currentWeight : 0.00;
            $monthWeightLossPercentage = ($monthWeightLoss != 0.00) ? round(($monthWeightLoss*100)/$MaxWeightMonth,0) : 0.00;
