	...
	<div class="container">
		<div class="payment-process">
					...
					<?php 
						if(isset($categories) && !empty($categories)){
							foreach ($categories as $key => $value) {
								echo $this->Form->input('category_id.'.$key,array('id' => 'checkbox'.$key, 'label' => false,'type' => 'checkbox','templates' => ['inputContainer' => '<div class="fleft width30"><div class="checkbox-div">{{content}}<label for="checkbox'.$key.'"></label></div><div class="customLabel">'.$value.'</div></div>']));
							}
						}
					?>
				</div>
			...
			<div class="row center-align mrg-40">
				<?= $this->Form->submit('Add Dish',['class' => 'green-btn']); ?>
			</div>
			<?= $this->Form->end(); ?>
			</div>
		</div>
