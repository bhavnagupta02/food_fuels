	...
	<div class="col-md-12">
		...
		<div class="portlet box blue">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-edit"></i>Meals (<?php echo $meal_count?>)
				</div>
			</div>
								...
								<?php
									echo $this->Html->link(
										'Add Meal <i class="fa fa-plus"></i>', [
											'controller' => 'DailyMealPlans', 'action' => 'add_meal', 'prefix' => 'backoffice', $plan_id
...
<script type="text/javascript">
    $(document).ready(function() { 
    	...
    	var dtttable = $('#clientsList').DataTable( {
    		"bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->Url->build(array('controller' => 'DailyMealPlans', 'action' => 'ajaxMeals',$plan_id)); ?>",
            "columnDefs": [
    	...
    	$('#clientsList tbody').on( 'click', 'a', function () {
	        ...
	        if(rel == 1){
	        	window.location = "<?= $this->Url->build(['controller' => 'DailyMealPlans', 'action' => 'edit_meal', 'prefix' => 'backoffice']); ?>/"+data[0];
	        }else if(rel == 2){
	        	if(confirm('Are you sure want to delete?')){
	        		window.location = "<?= $this->Url->build(['controller' => 'DailyMealPlans', 'action' => 'delete_meal', 'prefix' => 'backoffice']); ?>/"+data[0];
	        	}
	        }
	    });    
    });
