<!-- BEGIN PAGE HEADER-->
<h3 class="page-title"> Manage Recipes </h3>
<!-- END PAGE HEADER-->
<!-- BEGIN PAGE CONTENT-->
<div class="row">
	<div class="col-md-12">
		...
		<div class="portlet box blue">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-edit"></i>Recipes (<?php echo $templ_count?>)
				</div>
			</div>
...
<script type="text/javascript">
    $(document).ready(function() {
    	...
    	var dtttable = $('#clientsList').DataTable( {
    		"bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo $this->Url->build(array('controller' => 'recipes', 'action' => 'ajaxTemplates')); ?>",
            "columnDefs": [
	   ...
	   $('#clientsList tbody').on( 'click', 'a', function () {
	        ...
	        if(rel == 1){
	        	window.location = "<?= $this->Url->build(['controller' => 'recipes', 'action' => 'edit', 'prefix' => 'backoffice']); ?>/"+data[0];
	        }
	        else if(rel == 2){
	        	if(confirm('Are you sure want to delete?')){
	        		window.location = "<?= $this->Url->build(['controller' => 'recipes', 'action' => 'delete', 'prefix' => 'backoffice']); ?>/"+data[0];
	        	}
	        }
	    } );
