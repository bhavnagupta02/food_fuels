...
class UsersController extends AppController
{
	...
	public function beforeFilter(Event $event)
	{
		...
		$this->Auth->allow(['verify_user', 'verifyCaptcha', 'forgot_password',	'send_password', 'send_shared_emails','set_lang','enquiry','subscribe_me','skipVideo','cropnrotate','profile_upload','checkPayment', 'update_member_expired_date']);
			...
			if($afterDiscount == 0){
				...
				if ($save_user = $this->Users->save($user)){
					...
					$this->Auth->setUser($user->toArray());
                                        ...
                                        /* send membership renew notification email to admin */
					$tokenArray2 = ['{{user_name}}','{{user_email}}'];
					$tokenValueArray2 = [$this->Auth->user('first_name')." ".$this->Auth->user('last_name'), $this->Auth->user('email')];
					//$this->Common->_send_email('sean@foodfuelsweightloss.com', $tokenArray2, $tokenValueArray2, 'membership_renew_notification');  
