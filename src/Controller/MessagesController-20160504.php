...
class MessagesController extends AppController
{
	...
	public function detail($conversation_id)
	{
		...
		$this->ConversationReplies->updateAll(['seen' => 1],['conversation_id' => $conversation_id,'user_id !=' => $userId]);
 		...
 		$messages = $this->ConversationReplies->find()->where(['Conversations.id'=>$conversation_id, 'OR' => array('Conversations.sender_id'=>$userId, 'Conversations.receiver_id'=>$userId)])->contain(['Users','Conversations'])->order(['ConversationReplies.id' => 'ASC'])->all();
			...
			if(!empty($replyData['conversation_id']))
			{
				...
				if($this->ConversationReplies->save($conReply))
				{
					...
					$messages = $this
									...
									->order(['ConversationReplies.id' => 'ASC'])
									->all();
