 ...
 */
    public function getResponse($controller = '', $model = '')
    {
        ...
        $total = $this->model->find('all', ['conditions' => $conditions])->count();
        ...
        // execute sql select
        $data = $this->model->find('all', $parameters)->all();
 ...
 */
    private function getOrderByStatements(){
        ...
        //for ( $i=0 ; $i<intval( $this->controller->request->query['iSortingCols'] ) ; $i++ ){
            ...
            {
                $direction = $this->controller->request->query['sSortDir_0'] === 'asc' ? 'asc' : 'desc';
                $orderBy = $fields[ $this->controller->request->query['iSortCol_0']]['myModel'] . '.' . $fields[ $this->controller->request->query['iSortCol_0']]['db'] . ' ' . $direction . ', ';
            }
        //}
    ...
    private function getWhereConditions(){
        ...
        foreach($fields as $x => $column)
        {
            ...
            if( $this->controller->request->query['bSearchable_'.$x] == 'true' )
            {
                ...
                {
					$table = $column['myModel'];
					$field = $column['myModel'].'.'.$column['db'];
					$srhkey = $this->controller->request->query['sSearch'];
                    // attempt using definitions in $model->validate to build intelligent conditions
                    if(isset($fields[$x]['contain']))
					{
						$conditions['OR'][] = array("$field LIKE '%$srhkey%'");
					} else
					{
						$conditions['OR'][] = array("$field LIKE '%$srhkey%'");
					}
 ...
 */
    private function getDataRecursively($data, $key=null)
    {
		...
		{
			if ( isset( $column['contain'] ) )
			{
				$fields[ $column['dt'] ] = $data[$column['contain']][$column['db']];
			} else if ( isset( $column['formatter'] ) )
			{
				// Is there a formatter?
				$fields[ $column['dt'] ] = $column['formatter']( $data[ $column['db'] ], $data );
			} else
			{
				$fields[ $column['dt'] ] = $data[ $column['db'] ];
			}
		}
        return $fields;
    }
