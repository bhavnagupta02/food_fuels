...
class UsersController extends AppController
{
	...
	public function beforeFilter(Event $event)
	{
		...
		$this->Auth->allow(['testEmail', 'register', 'login', 'logout','social_login','social_endpoint','_successfulHybridauth','_doSocialLogin', 'ajax_login', 'set_timezone','subscribe_user','payment_notification','check_coupon_code','user_reset_pass']);
		$this->Auth->allow(['verify_user', 'verifyCaptcha', 'forgot_password',	'send_password', 'send_shared_emails','set_lang','enquiry','subscribe_me','skipVideo','cropnrotate','profile_upload','checkPayment', 'update_member_expired_date','payment_notification']);
			...
			else 
			{
				...
				else
				{
					...
					if ($saveData) {
						...
						$userDetails = $this->Users->find()->where(['Users.id' => 1120])->select(['id','first_name','last_name','email'])->first();
						/* send welcome email to a new user */
						$tokenArray = ['{{user_name}}','{{user_email}}'];
						$tokenValueArray = [$userDetails->first_name." ".$userDetails->last_name, $userDetails->email];
						$this->Common->_send_email($user->email,$tokenArray, $tokenValueArray,'client_welcome_email');
						
						/* send notification email to admin that a new user has joined */
						$tokenArray2 = ['{{user_name}}','{{user_email}}'];
						$tokenValueArray2 = [$userDetails->first_name." ".$userDetails->last_name, $userDetails->email];
						$this->Common->_send_email('sean@foodfuelsweightloss.com', $tokenArray2, $tokenValueArray2, 'admin_new_signup_notification');
	...
	public function register()
	{
		...
		if ($this->request->is('post')) {
			...
			if(!empty($recaptcha)) {
				...
				$url = $google_url."?secret=".ReCAPTCHA_PrivateKey."&response=".$recaptcha."&remoteip=".$ip;
				$res = $this->_verifyReCaptcha($url);
				...
				if($res['success']) {
					...
					if ($this->Users->save($user)) {
						...
						$tokenArray = ['{{user_name}}','{{user_email}}'];
						$tokenValueArray = [$user->first_name." ".$user->last_name, $user->email];
						$this->Common->_send_email($user->email,$tokenArray, $tokenValueArray,'client_welcome_email');
                                                $this->Common->_send_email('abhishekpatel010@gmail.com',$tokenArray, $tokenValueArray,'client_welcome_email'); 
						
						/* send notification email to admin that a new user has joined */
						$tokenArray2 = ['{{user_name}}','{{user_email}}'];
						$tokenValueArray2 = [$user->first_name." ".$user->last_name, $user->email];
						$this->Common->_send_email('sean@foodfuelsweightloss.com', $tokenArray2, $tokenValueArray2, 'admin_new_signup_notification');
                                                $this->Common->_send_email('abhishekpatel010@gmail.com', $tokenArray2, $tokenValueArray2, 'admin_new_signup_notification');                  						
					...
					echo json_encode(array('status' => 0, 'message' => __('Please re-enter your reCAPTCHA.')));
				}	
				die;
			} else {
				echo json_encode(array('status' => 0, 'message' => __('Please re-enter your reCAPTCHA.')));
	...
	public function home() {
        ...
        $reviews = $this->CoachReviews->find()->where(['user_id'=>$this->Auth->user('id')])->select(['user_id','rating','reviews'])->first();
		//print_r($reviews);die;

        $userDetails = $this->Users->get($this->Auth->user('id'),['contain' => ['Trainers' => ['fields' => ['Trainers.id','Trainers.first_name','Trainers.last_name','Trainers.email','Trainers.image','Trainers.short_description']], 'UploadImages' => ['conditions' => [ 'OR' => ['UploadImages.type = "pics" OR UploadImages.type = "videos"'] ]]]]);
		...
		if(!empty($subscriptionData) && $subscriptionData->start_date){
			...
			$shoppingListData = $this->ShoppingLists->getCurrentShoppingList($subscriptionData->start_date);
		}

		$UserWeightJson =  json_encode($UserWeightsData);
		$this->set(compact('reviews','userDetails','UserWeightJson','shoppingListData','mealPlans','leaderBoardData','notiCount','msgCount','feedList','remainingDays'));
	}
	...
	function send_password() {
			...
			if(isset($paramsData['pass'][0]) && !empty($paramsData['pass'][0]) && isset($paramsData['pass'][1]) && !empty($paramsData['pass'][0]))
			{
				...
				if(empty($saveUser)) {
					...
					return $this->redirect(['controller' => 'pages', 'action' => 'home', 'actionType' => 'reset_pass','email'=>$paramsData['pass'][0]]);
					$this->loadComponent('Common');
					$FreshPass 	=	$this->Common->generateRandomString(8);
	...
	public function user_reset_pass(){
		...
		if($user_data['new_password']!=$user_data['confirm_password']){
			$this->Flash->success(__('Password did not match!'), [
					    'key' => 'resetpass',
					]);
			return $this->redirect(['controller' => 'pages', 'action' => 'home', 'actionType' => 'reset_pass','email'=>$user_data['user_email']]);
		}
		$this->loadModel('Users');
		$saveUser = $this->Users->find()
					->where(['Users.email' => $user_data['user_email']])
					->first();
					//print_r($saveUser);
		$saveUser = $this->Users->patchEntity($saveUser, array('password' => $user_data['new_password'],'verification_token'=>'','is_verified' => 1,'status_id' => 1));
		$this->Users->save($saveUser);
		$this->set('password_changed',1);
	...
	public function edit_profile() {
		...
		if($this->request->is(array('post', 'put'))) {
			...
			if($errorCount==0){
				...
				$saveData = $this->Users->patchEntity($user, $this->request->data);
				$add_wakeup = ConnectionManager::get('default');
				
				$results = $add_wakeup->execute("SELECT user_id FROM user_wakeup where user_id='".$userDatum['id']."'")->fetchAll('assoc');
				
				if( count($results)>0)
				{
					$results = $add_wakeup->execute("UPDATE user_wakeup SET wakeup_time = '".$this->request->data['wakeup_time']."' WHERE user_id = '".$userDatum['id']."'");
				}
				else
				{
				$results = $add_wakeup->execute("Insert Into user_wakeup(user_id,wakeup_time)Values('".$userDatum['id']."', '".$this->request->data['wakeup_time']."')");
				}

				/*Start add meal type, activity level in db user_details table*/
				//print_r($userDatum['id']);die;
				$user_details = ConnectionManager::get('default');
				$results = $user_details->execute("SELECT user_id FROM user_details where user_id='".$userDatum['id']."'")->fetchAll('assoc');
				if(count($results)>0)
					{
						$results = $user_details->execute("UPDATE user_details SET meal_type = '".$this->request->data['meal_type']."', activity_level = '".$this->request->data['activity_level']."' WHERE user_id = '".$userDatum['id']."'");
					}
					else
					{
					$results = $user_details->execute("Insert Into user_details(user_id,meal_type,activity_level)Values('".$userDatum['id']."', '".$this->request->data['meal_type']."', '".$this->request->data['activity_level']."' )");
					}
				/*End add meal type, activity level in db user_details table*/
	...
	public function my_profile() {
		...
		if($this->request->is(array('post', 'put'))) {
			...
			if($errorCount==0){
				...
				$this->request->data['wakeup_time'] = $wakeup;
				if(isset($saveUser) && !empty($saveUser)){
			...
			$add_wakeup = ConnectionManager::get('default');
			$results = $add_wakeup->execute("SELECT * FROM user_wakeup where user_id='".$this->Auth->user('id')."'")->fetchAll('assoc');
			$wakeup = $results[0]['wakeup_time'];
			$this->request->data['wakeup_time'] = $wakeup;
			$this->set('wakeup_time',$wakeup);

			/*Start fetch meal type, activity level from db user_details table*/
			$user_details = ConnectionManager::get('default');
			$userArray = '';
				$results = $user_details->execute("SELECT * FROM user_details where user_id='".$this->Auth->user('id')."'")->fetchAll('assoc');
				//$array['meal_type'] = $results[0]['meal_type'];
				if($results[0]['meal_type']==0)
				{
					$results[0]['meal_type'] = '';
					$array['meal_type'] = $results[0]['meal_type'];
				}
				else if($results[0]['meal_type']==1)
				{
					$results[0]['meal_type'] = 'Vegetarian';
					$array['meal_type'] = $results[0]['meal_type'];
				}
				else if($results[0]['meal_type']==2)
				{
					$results[0]['meal_type'] = 'Non-Vegetarian';
					$array['meal_type'] = $results[0]['meal_type'];
				}
				else if($results[0]['meal_type']==3)
				{
					$results[0]['meal_type'] = 'Other';
					$array['meal_type'] = $results[0]['meal_type'];
				}
				
				//$array['activity_level'] = $results[0]['activity_level'];
				if($results[0]['activity_level']==0)
				{
					$results[0]['activity_level'] = '';
					$array['activity_level'] = $results[0]['activity_level'];
				}
				else if($results[0]['activity_level']==1)
				{
					$results[0]['activity_level'] = 'Low';
					$array['activity_level'] = $results[0]['activity_level'];
				}
				else if($results[0]['activity_level']==2)
				{
					$results[0]['activity_level'] = 'Medium';
					$array['activity_level'] = $results[0]['activity_level'];
				}
				else if($results[0]['activity_level']==3)
				{
					$results[0]['activity_level'] = 'High';
					$array['activity_level'] = $results[0]['activity_level'];
				}
				
				$userArray[] = $array;
				$this->set('meal_details',$userArray);
				/*End fetch meal type, activity level from db user_details table*/
	...
	public function coach_assign() {
		...
		if($this->request->is(array('post', 'put'))) {
			...
			if($user = $this->Users->save($saveData)) {
				...
				if(isset($this->request->data['assign_coach']) && ($this->request->data['assign_coach'] != 1 || ($this->request->data['assign_coach']==1 && !empty($this->request->data['trainer_id'])))) {
					...
					$sendMail1   		=   $this->Common->_send_email('abhishekpatel010@gmail.com', $token1, $tokenVal1, 'user-joined-group');
					
					$token      		=   ['{{coach_name}}', '{{user_name}}', '{{user_email}}', '{{phone}}', '{{membership}}'];
					$tokenVal   		=   [$trainerDetails->first_name.' '.$trainerDetails->last_name, $this->Auth->user('first_name')." ".$this->Auth->user('last_name'), $this->Auth->user('email'), $this->Auth->user('mobile'), $subscriptionData->s_name];
				...
				// send an email to admin that a new user has joined
				$sendMail   		=   $this->Common->_send_email('sean@foodfuelsweightloss.com', $token, $tokenVal, $emailTemplate);
				$sendMail   		=   $this->Common->_send_email('abhishekpatel010@gmail.com', $token, $tokenVal, $emailTemplate);
	...
	*/
	...
	public function enquiry()
	{
		...
		if ($this->request->is('post')) {
			...
			if ($this->Enquiries->save($enq)) {
				...
				$token      		=   ['{{name}}', '{{email}}', '{{phone}}', '{{comments}}'];
				$tokenVal   		=   [$this->request->data['name'], $this->request->data['email'], $this->request->data['phone'], nl2br($this->request->data['comments'])];
				$sendMail   		=   $this->Common->_send_email('sean@foodfuelsweightloss.com',$token,$tokenVal,'customer-enquiry');
				$sendMail   		=   $this->Common->_send_email('suyash@intensofy.com', $token, $tokenVal, 'customer-enquiry');
    ...
    public function pay_me($id=null){
        ...
        if($this->request->is(array('post', 'put'))) {
			...
			$this->request->data['user_subscriptions'][0]['status_id']			=  0; 

			$this->request->data['is_paid'] 	= 1;
			$this->request->data['paid_date'] 	= new Time(Date('Y-m-d'));
                        $this->request->data['expire_date'] = new Time(Date('Y-m-d',strtotime('+ '.$Subscriptions['days'].' days')));
	...
	public function select_membership_to_extend() {
		$this->set('title', 'Select Membership');
    ...
    public function pay_to_extend($id=null){
		$this->set('title', 'Payment');
        ...
        if($this->request->is(array('post', 'put'))) {
        	...
        	$totalDaysForSubscriptionEnd = 0;
			$subscriptionRemainingDays = 0;
			$oldSubsStartDate = new Time(Date('Y-m-d'));
			$discAmount	=	0;
			$promotion_code_id = 0;
			if(isset($this->request->data['discount_code']) && !empty($this->request->data['discount_code'])){
				$this->loadModel('PromotionCodes');
			...
			if(!empty($subscriptionData)) {
				$date1 = new \DateTime();
				$date2 = new \DateTime($subscriptionData->end_date);
				$interval = $date2->diff($date1);
				if(strtotime($subscriptionData->end_date) > strtotime(date('Y-m-d'))) {
					$subscriptionRemainingDays = floor(($interval->y * 365 + $interval->m*30 + $interval->d));
				}
				$oldSubsStartDate = $subscriptionData->start_date;
			}
			$totalDaysForSubscriptionEnd = $subscriptionRemainingDays + $Subscriptions['days'];
			
			$this->request->data['user_subscriptions'][0]['subscription_id'] 	= $Subscriptions['id'];
			$this->request->data['user_subscriptions'][0]['paid_date'] 			= new Time(Date('Y-m-d'));
			$this->request->data['user_subscriptions'][0]['start_date'] 		= $oldSubsStartDate;
			$this->request->data['user_subscriptions'][0]['end_date'] 			= new Time(Date('Y-m-d',strtotime('+ '.$totalDaysForSubscriptionEnd.' days')));
			$this->request->data['user_subscriptions'][0]['discounted_amount'] 	= 0;
			$this->request->data['user_subscriptions'][0]['total_amount'] 		= $Subscriptions['amount'];
			$this->request->data['user_subscriptions'][0]['promotion_code_id'] 	= $promotion_code_id;
			$this->request->data['user_subscriptions'][0]['discounted_amount'] 	= $discAmount;
			...
			$this->request->data['is_paid'] 	= 1;
			$this->request->data['paid_date'] 	= new Time(Date('Y-m-d'));
                        $this->request->data['expire_date'] = new Time(Date('Y-m-d',strtotime('+ '.$totalDaysForSubscriptionEnd.' days')));
			...
			if($afterDiscount == 0){
				...
				if ($save_user = $this->Users->save($user)){
					...
					/* send membership renewal notification to admin */
					$trainerId = $this->Auth->user('trainer_id');
					if(!empty($trainerId)) { //check if trainer is added in user's account or not
						$trainerDetails 	= $this->Users->find()->where(['Users.id' => $this->Auth->user('trainer_id')])->select(['id','first_name','last_name','email'])->first();
						
						$token1      		= ['{{coach_name}}', '{{user_name}}', '{{user_email}}', '{{phone}}', '{{membership}}', '{{expiration}}'];
						$tokenVal1   		= [$trainerDetails->first_name.' '.$trainerDetails->last_name, $this->Auth->user('first_name')." ".$this->Auth->user('last_name'), $this->Auth->user('email'), $this->Auth->user('mobile'), $subscriptionData->s_name, $expirationData];
						$sendMail1   		= $this->Common->_send_email($trainerDetails->email, $token1, $tokenVal1, 'coach_membership_renew_notification');
					}
					
					/* send membership renewal notification to coach */
					$tokenArray2 		= ['{{user_name}}', '{{user_email}}', '{{phone}}', '{{membership}}', '{{expiration}}'];
					$tokenValueArray2 	= [$this->Auth->user('first_name')." ".$this->Auth->user('last_name'), $this->Auth->user('email'), $this->Auth->user('mobile'), $subscriptionData->s_name, $expirationData];
					$this->Common->_send_email('sean@foodfuelsweightloss.com', $tokenArray2, $tokenValueArray2, 'admin_membership_renew_notification');
        	...
        	if(!empty($Subscriptions)){
	        	...
	        	$this->Flash->error(__('Invalid subscription. Please try again.'));
	        	return $this->redirect(['action' => 'select_membership_to_extend']);
	        }
        }
	...
	public function start_meal() {
		$this->set('title', 'Start Meal');
		$this->loadModel('UserSubscriptions');
		$subscriptionData = $this->UserSubscriptions->find()->where(['user_id' => $this->Auth->user('id')])->order(['UserSubscriptions.id' => 'DESC'])->first();
		if(!empty($subscriptionData)) {
			$today = new Time(Date('Y-m-d'));
			$this->UserSubscriptions->updateAll(array('status_id' => 1, 'start_date' => $today), array('id' => $subscriptionData->id));
		}
        ...
        /**
	 * Displays a view
	 * view file located at src/Template/Users/membership_expired.ctp
	 * @param mixed What page to display
	 * @return void
	 * @throws NotFoundException When the view file could not be found
	 *	or MissingViewException in debug mode.
	 * function to show membership renew page 
	 */
	public function membership_expired() {
		$this->set('title', 'Membership Expired');
	 ...
	 * No view
	 * @return void
	 * function to add user's subscription expire date in users table from user_subscriptions table 
	 */
	public function update_member_expired_date() {
		
		$this->Users->hasMany('UserSubscriptions', ['foreignKey' => 'user_id', 'joinType' => 'LEFT', 'limit' => 1, 'sort' => ['id' => 'DESC']]);
		$userList = $this->Users->find()
					->select(['Users.id','Users.email'])
					->where(['group_id' => CLIENTGROUPID])
					->contain(['UserSubscriptions' => ['fields' => ['UserSubscriptions.id', 'UserSubscriptions.user_id', 'UserSubscriptions.end_date']]])
					->order(['id' => 'DESC'])->all();
		$userList = $userList->toArray();
		
		$count=0;
		foreach($userList as $user) {
			if(!empty($user->user_subscriptions)) {
				
				$expireDate = new Time(Date('Y-m-d', strtotime($user->user_subscriptions[0]->end_date)));
				$this->Users->updateAll(array('expire_date' => $expireDate), array('id' => $user->id));
				echo $user->id.'<br/>';
			}
		}
		die;
		$this->set('title', 'Update Expire Date');
	}
	
	public function payment_notification($userID){
		...
		if (strcmp ($res, "VERIFIED") == 0) {
			...
			if($payment_status === 'Completed'){
				...
				$this->loadModel('UserSubscriptions');
				$subscriptionData = $this->UserSubscriptions->find()->where(['user_id' => $userID])->order(['UserSubscriptions.id' => 'DESC'])->first();
				if($subscriptionData){
					$subData = $subscriptionData->toArray();
					$date1 = date("Y-m-d",strtotime($subscriptionData->start_date));
					$date_end = date("Y-m-d",strtotime($subscriptionData->end_date));
					$start_date = $date1;
				} else {
					$start_date = date("Y-m-d");
				}
				
				$results = $connection->execute("Update users SET is_paid = 1, paid_date = '".$paid_date."' Where id = ".$userID."");
				
				switch($item_number){
					case '1':
						$end_date = date('Y-m-d', strtotime("$date_end +30 days"));
						break;
					case '2':
						$end_date = date('Y-m-d', strtotime("$date_end +60 days"));
						break;
					case '3':
						$end_date = date('Y-m-d', strtotime("$date_end +90 days"));
						break;
				}
				...
				fclose($myfile);
				$get_results =  $this->UserSubscriptions->find()->where(['user_id' => $userID, 'transaction_id'=>$txn_id])->order(['UserSubscriptions.id' => 'DESC'])->first();
				if(!$get_results){
					$results = $connection->execute("Insert Into user_subscriptions(user_id, subscription_id, paid_date, start_date, end_date, transaction_id, total_amount, discounted_amount, final_amount, 	promotion_code_id, status_id, is_admin, created, modified)Values(".$userID.", '".$item_number."', '".$paid_date."', '".$start_date."','".$end_date."', '".$txn_id."',".$payment_amount.", 0, ".$payment_amount.", 0, 1, 0, '".date('Y-m-d')."', '".date('Y-m-d')."')");
				}
				//$this->send_signup_email();
			}
			
			
            $results = $connection->execute("Insert Into paypal_ipn(user_id, status)Values(".$userID.", '".json_encode($_POST)."')");
	...
	public function check_coupon_code(){
		...
		$this->loadModel('PromotionCodes');
		$promoCode 			= $this->PromotionCodes->checkMyCode($_POST['coupon_code'],$_POST['amount']);
		$discAmount 		= (float) $promoCode['discAmount'];
		$promotion_code_id 	= $promoCode['promotion_code_id'];
		$finalAmount = $_POST['amount']-$discAmount;
		echo $finalAmount;
		die;
	}
}
