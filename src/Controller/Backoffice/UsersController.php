...
class UsersController extends AppController
{
	...
	public function weight($user_id = null)
	{
		...
		/*$weight_query = ConnectionManager::get('default');
		$results = $weight_query->execute("SELECT * FROM user_weights")->fetchAll('assoc');
		$this->set(compact('weight_count', 'results'));*/
	...
	public function ajaxClients()
	{
		...
		$conditions = ['Users.group_id' => CLIENTGROUPID];
	 	$this->paginate = array(
			...
			'order' => ['Users.id' => 'desc'],
			'fields' => array('Users.id', 'Users.first_name', 'Users.last_name', 'Users.username', 'Users.gender', 'Users.dob','Users.assign_coach','Users.leaderboard_show','Users.total_weight_loss_percent','Users.month_weight_loss_percent','Users.week_weight_loss_percent','Users.referred_by','Statuses.name', 'Users.mobile', 'Users.goal_weight', 'Users.is_paid', 'Users.email', 'Users.mobile', 'Users.address', 'Users.city', 'Users.zipcode', 'Countries.country', 'Users.created', 'Trainers.first_name', 'Trainers.last_name'),
		);
		$this->DataTable->fields = array(
			array( 'db' => 'id', 'dt' => 'id', 'myModel' => 'Users' ),
			array( 'db' => 'first_name', 'dt' => 'first_name', 'myModel' => 'Users' ),
			...
			array( 'db' => 'referred_by', 'dt' => 'referred_by' , 'myModel' => 'Users'),
...
public function multi_delete_weight()
{
    if (isset($this->request->data['multi_del']) && !empty($this->request->data['multi_del'])) 
    {
       ...
       // print_r($data_id_array);die;
        foreach ($data_id_array as $weight_id) {
		$this->loadModel('UserWeights');
		$user = $this->UserWeights->get($weight_id);
