...
class DailyMealPlansController extends AppController
{
	...
	public function add_meal($plan_id = null)
	{
		...
		if ($this->request->is('post'))
		{
			...
			{
				/*Start add gender, meal type, activity level in db daily_meal table*/
				$meal_id = $save_plan['id'];
				//print_r($meal_id);die;
				$add_meal = ConnectionManager::get('default');
				$results = $add_meal->execute("SELECT meal_id FROM daily_meal where meal_id='".$meal_id."'")->fetchAll('assoc');
				if(count($results)>0)
					{
						$results = $add_meal->execute("UPDATE daily_meal SET gender = '".$this->request->data['gender']."', meal_type = '".$this->request->data['meal_type']."', activity_level = '".$this->request->data['activity_level']."' WHERE meal_id = '".$meal_id."'");
					}
					else
					{
					$results = $add_meal->execute("Insert Into daily_meal(meal_id,gender,meal_type,activity_level)Values('".$meal_id."', '".$this->request->data['gender']."', '".$this->request->data['meal_type']."', '".$this->request->data['activity_level']."' )");
					}
				/*End ad gender, meal type, activity level in db daily_meal table*/

				//print_r($save_plan['id']);die;
				$this->Flash->success(__('This plan has successfully added.'),'success');
	...
	public function edit_meal($meal_id = null)
	{
		...
		//$this->set(compact('meal', 'statuses'));
		$this->set('_serialize', ['meal']);
	}	
