...
class DietsController extends AppController
{
			...
			if(strtotime($subscriptionData->end_date) >= strtotime($currentDate)) {	
				...
				if(!empty($mealPlans)) {
					...
					foreach ($mealPlans as $keyMeal => $row) {
						...
						//echo $weekNoForMealPlan."<br>";
						
						//echo $row->week_day."<br>";
						$week_no = ($weekNoForMealPlan);
	...
	public function index() {
		...
		if((!empty($subscriptionData) && $subscriptionData->start_date)){
			...
			if(strtotime($subscriptionData->end_date) >= strtotime($currentDate)) {	
				$this->loadModel('DailyMealPlans');
				$originalStartingWeek = $startingWeek = ceil($startingDays/7)==0?1:ceil($startingDays/7);
		...
		$data = $connection->execute("Select * From user_wakeup where user_id = '".$this->Auth->user('id')."'")->fetchAll('assoc');
		$user_wakeup_time = $data[0]['wakeup_time'];
		foreach($mealArray as $key => $each_date){
			
			foreach($each_date as $ikey=>$meal){

				foreach ($meal['meals'] as $mkey => $val) {
					switch ($val['heading']) {
						case 'Upon Waking Up':
							$mealArray[$key][$ikey]['meals'][$mkey]['time'] = $user_wakeup_time;
							break;
						case 'Meal 1':
							$date = date('h:i:s A', strtotime($user_wakeup_time)+1800);
							$mealArray[$key][$ikey]['meals'][$mkey]['time'] = $date;
							break;
						case 'Meal 2':
							$date = date('h:i:s A', strtotime($date)+10800);
							$mealArray[$key][$ikey]['meals'][$mkey]['time'] = $date;
							break;
						case 'Meal 3':
							$date = date('h:i:s A', strtotime($date)+10800);
	...
	public function new_meal_plan(){
		...
		$this->loadModel('UserSubscriptions');
		$subscriptionData = $this->UserSubscriptions->find()->where(['user_id' => $this->Auth->user('id')])->order(['UserSubscriptions.id' => 'DESC'])->first();
		echo $date_end = date("Y-m-d",strtotime($subscriptionData->end_date));
		echo date("Y-m-d",strtotime("$date_end +30 days"));
		$currentDate = date('Y-m-d');
