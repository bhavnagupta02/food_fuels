 ...
 *
 * This controller will render views from Template/Feeds/
 *
...
class FeedsController extends AppController
{
    ...
    public function index()
    {
        ...
        $this->set('title', 'Community');
        $feedList = $this->Feeds->getMyFeed('','',10,0);

        $feed_type = 'community';
        $feeds_data = ConnectionManager::get('default');
        $feedList1 = $feeds_data->execute("SELECT * FROM feeds a, feed_type b  where a.id=b.feed_id AND b.feed_type = '".$feed_type."'")->fetchAll('assoc');
        ...
        if ($this->request->is('post')) {
            ...
            if ($feeds = $this->Feeds->save($feeds)) {
                ...
                $results = $feed_details->execute("Insert Into feed_type (feed_id,feed_type)Values('".$feeds['id']."', 'community')");
        ...
        $this->set(compact('feedList', 'feedList1'));
    }
